...
static int64_t ModeIsDerefToROff(CRPN *m) {
  ...
  if (m->type == IC_DEREF) {
    ...
    if (m->type == IC_ADD || m->type == IC_SUB) {
      ...
      b = ICArgN(m, 1);
      // Dont do 123-reg
      if (m->type != IC_SUB && a->type == IC_IREG && IsConst(b) &&
          Is12Bit(ConstVal(b)))
        return 1;
      if (b->type == IC_IREG && IsConst(a) && Is12Bit(ConstVal(a)))
        return 1;
    }
  }
  return 0;
...
static int64_t PushToStack(CCmpCtrl *cctrl, CICArg *arg, char *bin,
  ...
  code_off = PutICArgIntoReg(cctrl, &tmp, tmp.raw_type, RISCV_IRET, bin, code_off);
...
static int64_t __ICMoveI64(CCmpCtrl *cctrl, int64_t reg, uint64_t imm,
  ...
  if (Is12Bit(imm)) {
    ...
    AIWNIOS_ADD_CODE(RISCV_LD(reg, RISCV_IRET, 0));
    if(bin) CodeMiscAddRef(misc, bin + code_off - 8);
  }
  return code_off;
}
...
found:
  AIWNIOS_ADD_CODE(RISCV_AUIPC(RISCV_IRET, 0));
  AIWNIOS_ADD_CODE(RISCV_FLD(reg, RISCV_IRET, 0));
  if(bin) CodeMiscAddRef(misc, bin + code_off - 8);
  return code_off;
}
  ...
  if (rpn2->type == IC_GLOBAL) {
    if (rpn2->global_var->base.type & HTT_FUN) {
      ...
      if(!fptr) 
		goto defacto;
    use_fptr:
      code_off = __ICMoveI64(cctrl, RISCV_IRET, fptr, bin, code_off);
      AIWNIOS_ADD_CODE(RISCV_JALR(1, RISCV_IRET, 0));
    }
  } else {
  defacto:
...
int64_t ICMov(CCmpCtrl *cctrl, CICArg *dst, CICArg *src, char *bin,
  ...
  case MD_CODE_MISC_PTR:
    if (src->mode == MD_REG &&
      ...
      case RT_U8i:
      case RT_I8i:
        AIWNIOS_ADD_CODE(RISCV_SB(src->reg, RISCV_IRET, 0));
        break;
      case RT_U16i:
      case RT_I16i:
        AIWNIOS_ADD_CODE(RISCV_SH(src->reg, RISCV_IRET, 0));
        break;
      case RT_U32i:
      case RT_I32i:
        AIWNIOS_ADD_CODE(RISCV_SW(src->reg, RISCV_IRET, 0));
        break;
      case RT_PTR:
      case RT_U64i:
      case RT_I64i:
      case RT_FUNC:
        AIWNIOS_ADD_CODE(RISCV_SD(src->reg, RISCV_IRET, 0));
        break;
      case RT_F64:
        AIWNIOS_ADD_CODE(RISCV_FSD(src->reg, RISCV_IRET, 0));
        break;
      default:
        abort();
      }
  ...
  case MD_STATIC:
    if ((src->raw_type == RT_F64) == (dst->raw_type == RT_F64) &&
        src->mode == MD_REG) {
      AIWNIOS_ADD_CODE(RISCV_AUIPC(RISCV_IRET, 0));
      AIWNIOS_ADD_CODE(RISCV_ADDI(RISCV_IRET,RISCV_IRET,0)); //Standard relocation opcods
      if (bin)
        CodeMiscAddRef(cctrl->statics_label, bin + code_off - 8)->offset=dst->off;
      switch (dst->raw_type) {
        break;
      case RT_U8i:
      case RT_I8i:
  ...
  indir_r2:
    ...
    dft:
      ...
      if (dst->raw_type == RT_F64 && src->raw_type != dst->raw_type) {
        ...
        if (src->mode == MD_REG) {
          AIWNIOS_ADD_CODE(RISCV_FCVT_L_D(use_reg, src->reg));
  ...
  store_r2:
    if (!Is12Bit(indir_off)) {
      code_off = __ICMoveI64(cctrl, RISCV_IRET, indir_off, bin, code_off);
      AIWNIOS_ADD_CODE(RISCV_ADD(RISCV_IPOOP2, RISCV_IRET, use_reg2));
      use_reg2  = RISCV_IPOOP2;
      indir_off = 0;
    }
    switch (dst->raw_type) {
    case RT_U0:
  ...
  case MD_REG:
    ...
    if (src->mode == MD_FRAME) {
      ...
      if ((src->raw_type == RT_F64) != (dst->raw_type == RT_F64))
        goto dft;
      AIWNIOS_ADD_CODE(RISCV_AUIPC(RISCV_IRET, 0));
      AIWNIOS_ADD_CODE(RISCV_ADDI(RISCV_ADDI, RISCV_IRET, 0));
      ...
      if (src->raw_type == RT_F64 && src->raw_type == dst->raw_type) {
        AIWNIOS_ADD_CODE(RISCV_SGNJ(use_reg, src->reg, src->reg))
      } else if (src->raw_type != RT_F64 && dst->raw_type != RT_F64) {
        AIWNIOS_ADD_CODE(RISCV_ADDI(use_reg, src->reg, 0))
      } else if (src->raw_type == RT_F64 && dst->raw_type != RT_F64) {
        goto dft;
      } else if (src->raw_type != RT_F64 && dst->raw_type == RT_F64) {
        goto dft;
    ...
    if(!Is12Bit(indir_off)) {
		code_off=__ICMoveI64(cctrl,RISCV_IRET,indir_off,bin,code_off);
		AIWNIOS_ADD_CODE(RISCV_ADD(RISCV_IRET,RISCV_IRET,use_reg2));
		indir_off=0;
	}
      switch (src->raw_type) {
      case RT_U0:
        break;
      case RT_U8i:
        AIWNIOS_ADD_CODE(RISCV_LBU(dst->reg, use_reg2, indir_off));
        break;
      case RT_I8i:
        AIWNIOS_ADD_CODE(RISCV_LB(dst->reg, use_reg2, indir_off));
        break;
      case RT_U16i:
        AIWNIOS_ADD_CODE(RISCV_LHU(dst->reg, use_reg2, indir_off));
        break;
      case RT_I16i:
        AIWNIOS_ADD_CODE(RISCV_LH(dst->reg, use_reg2, indir_off));
        break;
      case RT_U32i:
        AIWNIOS_ADD_CODE(RISCV_LWU(dst->reg, use_reg2, indir_off));
        break;
      case RT_I32i:
        AIWNIOS_ADD_CODE(RISCV_LW(dst->reg, use_reg2, indir_off));
        break;
      case RT_U64i:
      case RT_PTR:
      case RT_FUNC:
      case RT_I64i:
        AIWNIOS_ADD_CODE(RISCV_LD(dst->reg, use_reg2, indir_off));
        break;
      case RT_F64:
        AIWNIOS_ADD_CODE(RISCV_FLD(dst->reg, use_reg2, indir_off));
        break;
      default:
        abort();
      }
      ...
      if (dst->mode == MD_REG &&
        ...
        case RT_U8i:
        case RT_I8i:
          AIWNIOS_ADD_CODE(RISCV_LB(dst->reg, RISCV_IRET, 0));
          break;
        case RT_U16i:
        case RT_I16i:
          AIWNIOS_ADD_CODE(RISCV_LH(dst->reg, RISCV_IRET, 0));
          break;
        case RT_U32i:
        case RT_I32i:
          AIWNIOS_ADD_CODE(RISCV_LW(dst->reg, RISCV_IRET, 0));
          break;
        case RT_PTR:
        case RT_U64i:
        case RT_I64i:
        case RT_FUNC:
          AIWNIOS_ADD_CODE(RISCV_LD(dst->reg, RISCV_IRET, 0));
          break;
        case RT_F64:
          AIWNIOS_ADD_CODE(RISCV_FLD(dst->reg, RISCV_IRET, 0));
          break;
        default:
          abort();
        }
...
static int64_t DerefToICArg(CCmpCtrl *cctrl, CICArg *res, CRPN *rpn,
  ...
  if(rpn->type==IC_DEREF) {
	  if(ModeIsDerefToROff(rpn)) goto ent;
	  *res = rpn->res;
	  rpn=rpn->base.next;
	  code_off = __OptPassFinal(cctrl, rpn, bin, code_off);
      code_off = PutICArgIntoReg(cctrl, &rpn->res, RT_PTR, base_reg_fallback,
                               bin, code_off);
...
static int64_t __SexyPreOp(
  ...
  if (next->type == IC_TYPECAST) {
    ...
    if (next->raw_type == RT_F64) {
      ...
      AIWNIOS_ADD_CODE(freg(tmp.reg, tmp.reg, RISCV_FRET));
      ...
      if (Is12Bit(rpn->integer)) {
        AIWNIOS_ADD_CODE(i_imm(tmp.reg, tmp.reg, rpn->integer));
      } else {
        code_off = __ICMoveI64(cctrl, RISCV_IRET, rpn->integer, bin, code_off);
        AIWNIOS_ADD_CODE(ireg(tmp.reg, tmp.reg, RISCV_IRET));
      }
...
static int64_t RISCV_FJcc(CCodeMisc *to,int64_t cond,int64_t a,int64_t b,char *bin,int64_t code_off) {
	switch(cond) {
		case RISCV_COND_E:
		  AIWNIOS_ADD_CODE(RISCV_FEQ_D(RISCV_IRET,a,b))
		  code_off=RISCV_JccToLabel(to,RISCV_COND_E^1,RISCV_IRET,0,bin,code_off);
		  break;		
		case RISCV_COND_E^1:
		  AIWNIOS_ADD_CODE(RISCV_FEQ_D(RISCV_IRET,a,b))
		...
		case RISCV_COND_LE:
		case RISCV_COND_BE:
		  AIWNIOS_ADD_CODE(RISCV_FLE_D(RISCV_IRET,a,b))
		  code_off=RISCV_JccToLabel(to,RISCV_COND_E^1,RISCV_IRET,0,bin,code_off);
		  break;
		case RISCV_COND_L:
		case RISCV_COND_B:
		case RISCV_COND_AE^1:
		case RISCV_COND_GE^1:
		  AIWNIOS_ADD_CODE(RISCV_FLT_D(RISCV_IRET,a,b))
		  code_off=RISCV_JccToLabel(to,RISCV_COND_E^1,RISCV_IRET,0,bin,code_off);
		  break;
		case RISCV_COND_LE^1:
		case RISCV_COND_BE^1:
		case RISCV_COND_A:
		case RISCV_COND_G:
		  AIWNIOS_ADD_CODE(RISCV_FLT_D(RISCV_IRET,b,a))
		  code_off=RISCV_JccToLabel(to,RISCV_COND_E^1,RISCV_IRET,0,bin,code_off);
		  break;
		case RISCV_COND_L^1:
		case RISCV_COND_B^1:
		case RISCV_COND_AE:
		case RISCV_COND_GE:
		  AIWNIOS_ADD_CODE(RISCV_FLE_D(RISCV_IRET,b,a))
		  code_off=RISCV_JccToLabel(to,RISCV_COND_E^1,RISCV_IRET,0,bin,code_off);
		  break;
	}
	return code_off;
...
#define IC_MXX_X64(OP)                                                         \
  {                                                                            \
    ...
    into_reg     = rpn->res.mode == MD_REG ? rpn->res.reg : RISCV_IRET;        \
    tmp.mode     = MD_REG;                                                     \
    ...
    AIWNIOS_ADD_CODE(RISCV_ADDI(into_reg, b->res.reg, 0));                     \
    code_off = ICMov(cctrl, &rpn->res, &tmp, bin, code_off);                   \
  }
  ...
  ic_short_addr:
    ...
    AIWNIOS_ADD_CODE(RISCV_ADDI(into_reg, into_reg, 0));
    if (bin)
  ...
  ic_reloc:
    ...
    if (misc->aot_before_hint&&0) {
      ...
      AIWNIOS_ADD_CODE(RISCV_AUIPC(into_reg, i >> 12));
      AIWNIOS_ADD_CODE(RISCV_ADDI(into_reg, into_reg, i));
    } else  {
      AIWNIOS_ADD_CODE(RISCV_AUIPC(into_reg, 0));
      AIWNIOS_ADD_CODE(RISCV_LD(into_reg, into_reg, 0));
      if(bin) CodeMiscAddRef(rpn->code_misc, bin + code_off - 8);
    }
    if (rpn->res.mode != MD_REG) {
      tmp.mode     = MD_REG;
      tmp.raw_type = RT_I64i;
...
#define CMP_AND_JMP(COND)                                                      \
  {                                                                            \
    ...
    if (next3->raw_type != RT_F64 && next2->raw_type != RT_F64) {              \
      ...
      code_off = PutICArgIntoReg(cctrl, &next3->res, RT_I64i, RISCV_IPOOP1,    \
                                 bin, code_off);                               \
      code_off=RISCV_JccToLabel(rpn->code_misc,(COND)^reverse, next2->res.reg, next3->res.reg,bin,code_off);  \
      ...
      code_off = PutICArgIntoReg(cctrl, &next3->res, RT_F64, RISCV_FPOOP1,     \
                                 bin, code_off);                               \
      code_off=RISCV_FJcc(rpn->code_misc,(COND)^reverse, next2->res.reg, next3->res.reg,bin,code_off);  \
    }                                                                          \
  }
    ...
    code_off = __OptPassFinal(cctrl, next, bin, code_off);
    if (next->raw_type == RT_F64) {
      code_off = PutICArgIntoReg(cctrl, &next->res, RT_F64, RISCV_IPOOP1, bin, code_off);
      code_off = __ICMoveF64(cctrl, RISCV_FRET, 0, bin, code_off);
      AIWNIOS_ADD_CODE(RISCV_FEQ_D(RISCV_IRET, next->res.reg, RISCV_FRET));
      //freg==0.,so do opposite
      if (!reverse) {
		  code_off=RISCV_JccToLabel(rpn->code_misc,RISCV_COND_E,RISCV_IRET,0,bin,code_off);
      } else {
		  code_off=RISCV_JccToLabel(rpn->code_misc,RISCV_COND_E^1,RISCV_IRET,0,bin,code_off);
      }
    } else {
      code_off = PutICArgIntoReg(cctrl, &next->res, RT_I64i, RISCV_IRET, bin,
                                 code_off);
      if (!reverse) {
	 code_off=RISCV_JccToLabel(rpn->code_misc,RISCV_COND_E^1,next->res.reg,0,bin,code_off);
      } else {
	 code_off=RISCV_JccToLabel(rpn->code_misc,RISCV_COND_E,next->res.reg,0,bin,code_off);
      }
    }
    break;
  ic_to_bool:
    ...
    AIWNIOS_ADD_CODE(RISCV_SLTU(into_reg, 0, next->res.reg));
    code_off = ICMov(cctrl, &rpn->res, &tmp, bin, code_off);
    break;
  ...
  ic_typecast:
    ...
    if (next->raw_type == RT_F64 && rpn->raw_type != RT_F64) {
      ...
      if (rpn->res.mode == MD_REG)
        tmp.reg = rpn->res.reg;
      AIWNIOS_ADD_CODE(RISCV_FMV_X_D(tmp.reg, next->res.reg));
      if (rpn->res.keep_in_tmp)
        rpn->res = next->res;
      code_off = ICMov(cctrl, &rpn->res, &tmp, bin, code_off);
      ...
      if (rpn->res.mode == MD_REG)
        tmp.reg = rpn->res.reg;
      AIWNIOS_ADD_CODE(RISCV_FMV_D_X(tmp.reg, next->res.reg));
      if (rpn->res.keep_in_tmp)
        rpn->res = next->res;
      code_off = ICMov(cctrl, &rpn->res, &tmp, bin, code_off);
...
#define BACKEND_UNOP(flt_op, int_op)                                              \
  ...
  jmp_tab_sexy:
    AIWNIOS_ADD_CODE(RISCV_AUIPC(RISCV_IRET, 0));
    AIWNIOS_ADD_CODE(RISCV_ADDI(RISCV_IRET, RISCV_IRET, 0));
    CodeMiscAddRef(rpn->code_misc, bin + code_off - 8);
    // IPOOP2 has lower bound
    AIWNIOS_ADD_CODE(RISCV_SUB(RISCV_IPOOP1, RISCV_IPOOP1, RISCV_IPOOP2));
    AIWNIOS_ADD_CODE(RISCV_SLLI(RISCV_IPOOP1, RISCV_IPOOP1, 3)); //*8
    AIWNIOS_ADD_CODE(RISCV_ADD(RISCV_IPOOP1, RISCV_IPOOP1, RISCV_IRET));
    AIWNIOS_ADD_CODE(RISCV_LD(RISCV_IPOOP1,RISCV_IPOOP1,0));
  ...
  ic_sub_call:
    ...
    AIWNIOS_ADD_CODE(RISCV_AUIPC(RISCV_IRET, 0));//ADRESS STARTS HERE+4(1)
    AIWNIOS_ADD_CODE(RISCV_ADDI(RISCV_IPOOP1,RISCV_IRET,4*6)); //+4(2)
    AIWNIOS_ADD_CODE(RISCV_SD(RISCV_IPOOP1,RISCV_REG_SP,0)); //+4(2)
    AIWNIOS_ADD_CODE(RISCV_AUIPC(RISCV_IRET, 0));//+4(3)
    AIWNIOS_ADD_CODE(RISCV_ADDI(RISCV_IRET, RISCV_IRET, 0));//+4(4)
    CodeMiscAddRef(rpn->code_misc, bin + code_off - 8);
  ...
  ic_eq:
    ...
    if (next->type == IC_TYPECAST) {
      ...
      if (rpn->res.mode != MD_NULL) {
        // Avoid a double-derefernce
        code_off = PutICArgIntoReg(cctrl, &next2->res, rpn->raw_type,
                                   rpn->raw_type == RT_F64 ? RISCV_FPOOP1 : RISCV_IPOOP1,
                                   bin, code_off);
      }
      code_off = ICMov(cctrl, &tmp, &next2->res, bin, code_off);
      code_off = ICMov(cctrl, &rpn->res, &tmp, bin, code_off);
      ...
      if (IsConst(next))
        if (__builtin_popcountll(next->integer) == 1) {
          ...
          default:
            ...
            tmp.reg      = into_reg;
            code_off     = ICMov(cctrl, &tmp, &next2->res, bin, code_off);
            AIWNIOS_ADD_CODE(RISCV_SRLI(into_reg, into_reg,
  ...
  ic_mul:
    ...
    if (next->type == IC_I64 && next2->raw_type != RT_F64) {
      if (__builtin_popcountll(next->integer) == 1) {
        ...
        if (rpn->res.mode == MD_REG) {
          code_off = ICMov(cctrl, &rpn->res, &next2->res, bin, code_off);
          AIWNIOS_ADD_CODE(RISCV_SLLI(rpn->res.reg, rpn->res.reg,
                                      __builtin_ffsll(next->integer) - 1));
        } else {
          tmp.raw_type = RT_I64i;
          tmp.reg      = RISCV_IRET;
          tmp.mode     = MD_REG;
          code_off     = ICMov(cctrl, &tmp, &next2->res, bin, code_off);
          AIWNIOS_ADD_CODE(RISCV_SLLI(tmp.reg, tmp.reg,
                                      __builtin_ffsll(next->integer) - 1));
          if (rpn->res.keep_in_tmp)
            rpn->res = tmp;
          code_off = ICMov(cctrl, &rpn->res, &tmp, bin, code_off);
...
#define FBITWISE(i_op)                                                         \
  {                                                                            \
    ...
    code_off = PutICArgIntoReg(cctrl, &next->res, RT_F64, RISCV_FPOOP2, bin,   \
                               code_off);                                      \
    AIWNIOS_ADD_CODE(RISCV_FMV_D_X(RISCV_IPOOP1, RISCV_FPOOP1));               \
    AIWNIOS_ADD_CODE(RISCV_FMV_D_X(RISCV_IPOOP2, RISCV_FPOOP2));               \
    AIWNIOS_ADD_CODE(i_op(RISCV_IPOOP1, RISCV_IPOOP1, RISCV_IPOOP2));          \
    tmp.raw_type = RT_F64;                                                     \
    tmp.reg      = RISCV_IPOOP1;                                               \
    tmp.mode     = MD_REG;                                                     \
    code_off     = ICMov(cctrl, &rpn->res, &tmp, bin, code_off);               \
  ...
  ic_addr_of:
    ...
    case IC_BASE_PTR:
      ...
      if (Is12Bit(-next->integer)) {
        AIWNIOS_ADD_CODE(RISCV_ADDI(into_reg, RISCV_REG_FP, -next->integer));
      } else {
        code_off = __ICMoveI64(cctrl, into_reg, -next->integer, bin, code_off);
        AIWNIOS_ADD_CODE(RISCV_ADD(into_reg, RISCV_REG_FP, into_reg));
      }
      goto restore_reg;
      break;
    case IC_STATIC:
  ...
  ic_mod:
    ...
    if (rpn->raw_type != RT_F64) {
      ...
      AIWNIOS_ADD_CODE(RISCV_FDIV_D(tmp2.reg, next->res.reg, next2->res.reg));
      tmp.reg      = RISCV_IRET;
      tmp.mode     = MD_REG;
      tmp.raw_type = RT_I64i;
      code_off     = ICMov(cctrl, &tmp, &tmp2, bin, code_off);
      code_off     = ICMov(cctrl, &tmp2, &tmp, bin, code_off);
      AIWNIOS_ADD_CODE(RISCV_FMUL_D(RISCV_FPOOP2, tmp2.reg, next2->res.reg));
      AIWNIOS_ADD_CODE(RISCV_FSUB_D(tmp2.reg, next->res.reg, RISCV_FPOOP2));
    ...
    for (i = cnt - 1; i >= 0; i--) {
      ...
      if (range_fail_regs[i].is_flt) {
        // We used RISCV_Fcc_D to set RISCV_IRET,just check if not 0
                    code_off=RISCV_FJcc(old_fail_misc?old_fail_misc:rpn->code_misc,range_cmp_types[i], tmp.reg, tmp2.reg,bin,code_off);

      } else {
        code_off=RISCV_JccToLabel(old_fail_misc?old_fail_misc:rpn->code_misc,range_cmp_types[i], range_fail_regs[i].a,
                                   range_fail_regs[i].b, bin,code_off);
      }
      next = next2;
    }
  ...
  ic_lnot:
    ...
    if (old_pass_misc && old_fail_misc) {
	  code_off=RISCV_JccToLabel(old_pass_misc,RISCV_COND_E,next->res.reg,0,bin,code_off);
      AIWNIOS_ADD_CODE(RISCV_JAL(0, 0));
      if (bin)
  ...
  ic_post_inc:
    code_off =
        __SexyPostOp(cctrl, rpn, RISCV_ADDI, RISCV_ADD, RISCV_FADD_D, bin, code_off);
    break;
  ic_post_dec:
    code_off =
        __SexyPostOp(cctrl, rpn, RISCV_SUBI, RISCV_SUB, RISCV_FSUB_D, bin, code_off);
    break;
  ic_pre_inc:
    code_off =
        __SexyPreOp(cctrl, rpn, RISCV_ADDI, RISCV_ADD, RISCV_FADD_D, bin, code_off);
    break;
  ic_pre_dec:
    code_off =
        __SexyPreOp(cctrl, rpn, RISCV_SUBI, RISCV_SUB, RISCV_FSUB_D, bin, code_off);
    break;
  ic_and_and:
#define F64_TO_BOOL_IF_NEEDED(_mode, fallback)                                  \
  ...
  ic_or_or:
    if (old_pass_misc || old_fail_misc) {
      ...
      code_off=RISCV_JccToLabel(old_pass_misc,RISCV_COND_E^1,b->res.reg, 0,bin,code_off);
      AIWNIOS_ADD_CODE(RISCV_JAL(0, 0));
      CodeMiscAddRef(old_fail_misc, bin + code_off - 4)->is_jal = 1;
      goto ret;
    }
...
#define BACKEND_CMP(cond)                                                      \
  ...
  if (old_pass_misc && old_fail_misc) {                                        \
    if (rpn->raw_type != RT_F64) {                                             \
      	 code_off=RISCV_JccToLabel(old_pass_misc,cond,next->res.reg, next2->res.reg,bin,code_off);\
      AIWNIOS_ADD_CODE(RISCV_JAL(0, 0));                                       \
      CodeMiscAddRef(old_fail_misc, bin + code_off - 4)->is_jal = 1;           \
    } else {                                                                   \
      AIWNIOS_ADD_CODE(                                                        \
          RISCV_FEQ_D(RISCV_IRET, next->res.reg, next2->res.reg));             \
      	 code_off=RISCV_JccToLabel(old_pass_misc,(cond)^1,RISCV_IRET, 0,bin,code_off);\
      AIWNIOS_ADD_CODE(RISCV_JAL(0, 0));                                       \
      CodeMiscAddRef(old_fail_misc, bin + code_off - 4)->is_jal = 1;           \
    }                                                                          \
  } else {                                                                     \
    if (rpn->raw_type != RT_F64) {                                             \
      ...
      if ((cond) == RISCV_COND_E) {                                            \
        ...
        AIWNIOS_ADD_CODE(RISCV_SLTU(RISCV_IRET, 0, RISCV_IRET));               \
    } else {                                                                   \
      AIWNIOS_ADD_CODE(                                                        \
          RISCV_FEQ_D(RISCV_IRET, next->res.reg, next2->res.reg));             \
    ...
    if (rpn->raw_type != RT_F64) {
      ...
      if (IsConst(next2) && Is12Bit(ConstVal(next2))) {
        ...
        code_off = PutICArgIntoReg(cctrl, &next->res, RT_U64i, RISCV_IPOOP2,
                                   bin, code_off);
        AIWNIOS_ADD_CODE(
            RISCV_SLLI(RISCV_IPOOP1, next->res.reg, ConstVal(next2)));
        goto lsfin;
      }
      code_off = __OptPassFinal(cctrl, next, bin, code_off);
      code_off = __OptPassFinal(cctrl, next2, bin, code_off);
      ...
      code_off = PutICArgIntoReg(cctrl, &next->res, RT_U64i, RISCV_IPOOP2, bin,
                                 code_off);
      AIWNIOS_ADD_CODE(RISCV_SLL(RISCV_IPOOP1, next->res.reg, next2->res.reg));
    lsfin:
      tmp2.mode     = MD_REG;
  ...
  ic_rsh:;
    ...
    if (rpn->raw_type != RT_F64) {
      ...
      if (IsConst(next2) && Is12Bit(ConstVal(next2))) {
        ...
        if (_signed)
          AIWNIOS_ADD_CODE(
              RISCV_SRAI(RISCV_IPOOP1, next->res.reg, ConstVal(next2)))
        else
          AIWNIOS_ADD_CODE(
              RISCV_SRLI(RISCV_IPOOP1, next->res.reg, ConstVal(next2)))
        goto rsfin;
      }
      code_off = __OptPassFinal(cctrl, next, bin, code_off);
      code_off = __OptPassFinal(cctrl, next2, bin, code_off);
      ...
      if (_signed)
        AIWNIOS_ADD_CODE(
            RISCV_SRA(RISCV_IPOOP1, next->res.reg, next2->res.reg))
      else
        AIWNIOS_ADD_CODE(
            RISCV_SRL(RISCV_IPOOP1, next->res.reg, next2->res.reg))
    rsfin:
      tmp2.mode     = MD_REG;
      tmp2.raw_type = RT_I64i;
      tmp2.reg      = RISCV_IPOOP1;
  ...
  ic_ret:
    ...
    if (tmp.raw_type == RT_F64) {
      AIWNIOS_ADD_CODE(RISCV_FMV_X_D(RISCV_IRET, RISCV_FRET));
    } else {
      // Vise versa
      AIWNIOS_ADD_CODE(RISCV_FMV_D_X(RISCV_IRET, RISCV_FRET));
    }
  ...
  if (old_fail_misc || old_pass_misc) {
    ...
    default:
      if (rpn->res.raw_type == RT_F64) {
        ...
        // Pass address
         code_off=RISCV_JccToLabel(old_pass_misc,RISCV_COND_E ^ 1, tmp.reg, 0,bin,code_off);
      }
      // Fail address
      AIWNIOS_ADD_CODE(RISCV_JAL(0, 0));
      CodeMiscAddRef(old_fail_misc, bin + code_off - 4)->is_jal = 1;
            ...
            if (cm_ref->is_abs64) {\
              if (misc->patch_addr)\
                *misc->patch_addr = cm_ref->add_to;\
				...
				int64_t b4_1=(idx>>1)&((1<<4)-1);\
				int64_t b10_5=(idx>>5)&((1<<6)-1);\
				*(int32_t*)(cm_ref->add_to)|=(b12<<31)|(b10_5<<25)|(b4_1<<8)|(b4_1<<8)|(b11<<7);\
				...
				*(int32_t*)(cm_ref->add_to)|=(b20<<31)|(b10_1<<21)|(b11<<20)|(b19_12<<12);\
			} else {\
				int64_t low12;\
				idx=(char *) misc->addr - (char *)cm_ref->add_to+cm_ref->offset;\
