...
//
//
// is_terminal - will tell you the the code ends the control flow
//
// returns 1 if deteched a change
//
// 1+2; is_terminal==0
// goto a; is_terminal==1
// 3+4; is_terminal==0
// return 1; is_terminal==1
//
static int64_t __OptPassDCE0(CCmpCtrl* cctrl, CRPN* rpn, int64_t* is_terminal)
{
re_enter:;
	CRPN *cur, *head, *fin;
	int64_t changed = 0, is_term2 = 0;
	int64_t idx;
#define MARK_ALIVE(rpn)               \
	if ((rpn)->flags & ICF_DEAD_CODE) \
		changed = 1;                  \
	(rpn)->flags &= ~ICF_DEAD_CODE;
	if (is_terminal)
		*is_terminal = 0;
	MARK_ALIVE(rpn);
	switch (rpn->type) {
	...
	case IC_GOTO:
	case IC_SUB_CALL:
		head = cctrl->code_ctrl->ir_code;
		for (cur = head->base.next; cur != head; cur = cur->base.next) {
			if (cur->type == IC_LABEL && cur->code_misc == rpn->code_misc)
				if (cur->flags & ICF_DEAD_CODE) {
					is_term2 = 0;
					changed |= __OptPassDCE0(cctrl, cur, &is_term2);
					break;
				}
		}
		if (rpn->type == IC_GOTO && is_terminal)
			*is_terminal = 1;
		break;
	gfin:
		break;
	...
	case IC_UNBOUNDED_SWITCH:
		rpn->flags &= ~ICF_DEAD_CODE;
		head = cctrl->code_ctrl->ir_code;
		for (idx = 0; idx <= rpn->code_misc->hi - rpn->code_misc->lo; idx++) {
			for (cur = head->base.next; cur != head; cur = cur->base.next) {
				if (cur->type == IC_LABEL && cur->code_misc == rpn->code_misc->jmp_tab[idx])
					if (cur->flags & ICF_DEAD_CODE) {
						is_term2 = 0;
						changed |= __OptPassDCE0(cctrl, cur, &is_term2);
						break;
		...
		for (cur = head->base.next; cur != head; cur = cur->base.next) {
			if (cur->type == IC_LABEL && cur->code_misc == rpn->code_misc->dft_lab)
				if (cur->flags & ICF_DEAD_CODE) {
					is_term2 = 0;
					changed |= __OptPassDCE0(cctrl, cur, &is_term2);
					break;
	...
	case IC_GOTO_IF:
		...
		goto dft;
	case IC_RET:
		changed |= __OptPassDCE0(cctrl, rpn->base.next, NULL);
		if (is_terminal)
			*is_terminal = 1;
		break;
	case IC_LABEL:
	...
	dft:
		fin = ICFwd(rpn);
		for (cur = rpn->base.next; cur != fin; cur = cur->base.next) {
			MARK_ALIVE(cur);
		}
	}
	return changed;
}
void OptPassDeadCodeElim(CCmpCtrl* cctrl)
{
	CRPN *cur, *head = cctrl->code_ctrl->ir_code, *next;
	int64_t is_term = 0, cnt = 0, cnt2;
	for (cur = head->base.next; cur != head; cur = cur->base.next) {
		cur->flags |= ICF_DEAD_CODE;
	}
	//
	// Because this is reverse polish notation,we must "forward" it
	//
	for (cur = head->base.next; cur != head; cur = ICFwd(cur))
		cnt++;
	CRPN* forwards[cnt2 = cnt];
	cnt = 0;
	for (cur = head->base.next; cur != head; cur = ICFwd(cur)) {
		forwards[cnt2 - ++cnt] = cur;
	}
	...
	pass:
		is_term = 0;
		__OptPassDCE0(cctrl, cur, &is_term);
	}
	for (cur = head->base.next; cur != head; cur = next) {
		next = cur->base.next;
		if (cur->flags & ICF_DEAD_CODE)
			ICFree(cur);
	}
