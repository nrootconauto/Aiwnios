	...
	int64_t code_off)
{
  ...
  switch(r) {
    break;case RT_I8i: rsz=1;
    break;case RT_U8i: rsz=1;
    break;case RT_I16i: rsz=2;
    break;case RT_U16i: rsz=2;
    break;case RT_I32i: rsz=4;
    break;case RT_U32i: rsz=4;
    break;default: rsz=8;
	...
	int64_t code_off)
{
  ...
  case MD_STATIC:
    ...
    indir_off=0;
    if(cctrl->code_ctrl->final_pass>=2) {
      ARM_adrX(use_reg2,dst->off-code_off+cctrl->code_ctrl->statics_offset);
    } else
      AIWNIOS_ADD_CODE(0);
    goto indir_r2;
		break;
	...
	char* bin, int64_t code_off)
{
	...
	CICArg derefed, tmp, tmp2;
	if (next->type == IC_TYPECAST) {
#define TYPECAST_ASSIGN_BEGIN(DST)                                                         \
	{                                                                                      \
...
#define TYPECAST_ASSIGN_END(DST)                               \
	...
	if (pop2)                                                  \
		...
		if (next->raw_type == RT_F64) {
			...
			AIWNIOS_ADD_CODE(i_imm_op(next->res.reg, next->res.reg, rpn->integer));
			code_off = ICMov(cctrl, &orig, &next->res, bin, code_off);
			code_off = ICMov(cctrl, &rpn->res, &next->res, bin, code_off);
		}
		...
		if (next->raw_type == RT_F64) {
			...
			tmp = derefed;
			code_off = PutICArgIntoReg(cctrl, &tmp, tmp.raw_type, 2, bin, code_off);
			AIWNIOS_ADD_CODE(i_imm_op(tmp.reg, tmp.reg, rpn->integer));
			code_off = ICMov(cctrl, &derefed, &tmp, bin, code_off);
			code_off = ICMov(cctrl, &rpn->res, &tmp, bin, code_off);
		}
	}
	...
	int64_t shift, char* bin, int64_t code_off)
{
	...
	if (next->type == IC_TYPECAST) {
		tc = 1;
		next = rpn->base.next;
		TYPECAST_ASSIGN_BEGIN(next2);
		goto enter;
	tc_reenter:;
		TYPECAST_ASSIGN_END(next2);
		return code_off;
	}
enter:
...
{
	CRPN *next, *next2, *tc;
	CICArg tmp, orig_dst, derefed, tmp2;
	int64_t into_reg;
	next = ICArgN(rpn, 1);
	next2 = ICArgN(rpn, 0);
	if (SpillsTmpRegs(next))
...
#define BACKEND_BIT_BINOP(bit_op, shift)                                     \
	...
	case IC_BNOT:
		...
		break;
	case IC_POST_INC:
		code_off = __SexyPostOp(cctrl, rpn, ARM_addImmX, ARM_faddReg, bin, code_off);
		break;
	case IC_POST_DEC:
		code_off = __SexyPostOp(cctrl, rpn, ARM_subImmX, ARM_fsubReg, bin, code_off);
		break;
	case IC_PRE_INC:
		code_off = __SexyPreOp(cctrl, rpn, ARM_addImmX, ARM_faddReg, bin, code_off);
		break;
	case IC_PRE_DEC:
		code_off = __SexyPreOp(cctrl, rpn, ARM_subImmX, ARM_fsubReg, bin, code_off);
		break;
	case IC_AND_AND:
  ...
  case __IC_CALL:
    ...
    //  \   ^   |    |__IFCall            |
      ...
      if(tmp.raw_type<RT_I64i)
		...
		break;
	case IC_ADD_EQ:
		code_off = __SexyAssignOp(cctrl, rpn, &ARM_addRegX, &ARM_faddReg, bin, code_off);
		break;
	case IC_SUB_EQ:
		code_off = __SexyAssignOp(cctrl, rpn, &ARM_subRegX, &ARM_fsubReg, bin, code_off);
		break;
	case IC_MUL_EQ:
		code_off = __SexyAssignOp(cctrl, rpn, &ARM_mulRegX, &ARM_fmulReg, bin, code_off);
		break;
	case IC_DIV_EQ:
		code_off = __SexyAssignOp(cctrl, rpn, &ARM_sdivRegX, &ARM_fdivReg, bin, code_off);
		break;
	case IC_LSH_EQ:
		if (rpn->raw_type == RT_F64)
			abort();
		code_off = __SexyAssignOp(cctrl, rpn, &ARM_lslvRegX, &ARM_lslvRegX, bin, code_off);
		break;
	case IC_RSH_EQ:
		if (rpn->raw_type == RT_F64)
			abort();
		...
		case RT_U32i:
		case RT_U64i:
			code_off = __SexyAssignOp(cctrl, rpn, &ARM_lsrvRegX, &ARM_lsrvRegX, bin,
				code_off);
			break;
		default:
			code_off = __SexyAssignOp(cctrl, rpn, &ARM_asrvRegX, &ARM_asrvRegX, bin,
				code_off);
		}
		break;
	case IC_AND_EQ:
		if (rpn->raw_type == RT_F64)
			abort(); // TODO
		code_off = __SexyAssignOp(cctrl, rpn, &ARM_andRegX, &ARM_andRegX, bin, code_off);
		break;
	case IC_OR_EQ:
		if (rpn->raw_type == RT_F64)
			abort(); // TODO
		code_off = __SexyAssignOp(cctrl, rpn, &ARM_orrRegX, &ARM_orrRegX, bin, code_off);
		break;
	case IC_XOR_EQ:
		if (rpn->raw_type == RT_F64)
			abort(); // TODO
		code_off = __SexyAssignOp(cctrl, rpn, &ARM_eorRegX, &ARM_eorRegX, bin, code_off);
		break;
	case IC_ARROW:
		abort();
		break;
...
char* OptPassFinal(CCmpCtrl* cctrl,int64_t *res_sz,char **dbg_info)
{
  ...
  for(r = cctrl->code_ctrl->ir_code->next; r != cctrl->code_ctrl->ir_code; r =r->base.next) {
    if(r->ic_line) {
      if(!min_ln) min_ln=r->ic_line;
      min_ln=(min_ln<r->ic_line)?min_ln:r->ic_line;
      if(!max_ln) max_ln=r->ic_line;
